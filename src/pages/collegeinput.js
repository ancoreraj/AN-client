...
const CollegeInput = () => {
    ...
    const [college,setCollege]=useState("what")
    ...
    useEffect(()=>{
        ...
        const college=JSON.parse(localStorage.getItem("user")).collegeName
        if(college){
            setCollege(college)
        }
    },[])
                        ...
                        <form onSubmit={(e) => {
                            ...
                            <div class="form-group m-1 go">
                                <select onfocus='this.size=10' onblur='this.size=1'
                                    onchange='this.size=1; this.blur();' class="form-control collegeSelect" name="college" required value={college}>
